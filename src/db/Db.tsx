...
export class Contact {
  ...
  async loadNavigationProperties() {
      [this.emails, this.phones] = await Promise.all([
          db.emails.where('contactId').equals(this.id as IndexableType).toArray(),
          db.phones.where('contactId').equals(this.id as IndexableType).toArray()
      ]);
  }
      ...
      return db.transaction('rw', db.contacts, db.emails, db.phones, async() => {
          ...
          await Promise.all([
              db.emails.where('contactId').equals(this.id) // references us
                  .and(email => emailIds.indexOf(email.id as number) === -1) // Not anymore in our array
                  .delete(),
          
              db.phones.where('contactId').equals(this.id)
                  .and(phone => phoneIds.indexOf(phone.id as number) === -1)
                  .delete()
          ]);
      });
  }
